...
public class FileSystem extends org.apache.hadoop.fs.FileSystem {
    ...
    static java.nio.file.FileSystem jimfs = null;//Jimfs.newFileSystem(com.google.common.jimfs.Configuration.unix());
    ...
    public static void printAllFile(java.nio.file.Path root, java.nio.file.FileSystem fs) throws IOException{
    ...
    public FSDataInputStream open(Path path, int i) throws IOException {
        ...
        FileChannel channel = FileChannel.open(jimfs.getPath(path.toString().substring(path.toString().indexOf(":") + 1)), StandardOpenOption.READ);
    ...
    private FSDataOutputStream createInternal(Path path, FsPermission fsPermission,
        ...
        java.nio.file.Path nioFilePath = jimfs.getPath(path.toString().substring(path.toString().indexOf(":") + 1));
    ...
    public boolean delete(Path path, boolean b) throws IOException {
        try (val lock = this.lock.open()) {
            return this.deleteInternal(jimfs.getPath(path.toString().substring(path.toString().indexOf(":") + 1)), b);
        }
    }
    ...
    public boolean renameInternal(Path path, Path path1) throws IOException {
        ...
        java.nio.file.Path nioFilePath = jimfs.getPath(path.toString().substring(path.toString().indexOf(":") + 1));
        java.nio.file.Path nioFilePath1 = jimfs.getPath(path1.toString().substring(path1.toString().indexOf(":") + 1));
        java.nio.file.Path nioParentDirPath = nioFilePath.getParent();
        Files.createDirectories(nioParentDirPath);
    ...
    public FileStatus[] listStatus(Path path) throws FileNotFoundException, IOException {
        ...
        try (val lock = this.lock.open()) {
            ...
            if (Files.isDirectory(nioFilePath)) {
                DirectoryStream<java.nio.file.Path> stream = Files.newDirectoryStream(nioFilePath);
                for (java.nio.file.Path p : stream) {
                    result.add(new FileStatus(Files.size(p), Files.isDirectory(p), 1, 1, 0, 0,
                            new FsPermission((short) 777), null, null, new Path(SCHEME + ":/" + p.toString())));
                }
                stream.close();
            }
        }
    ...
    private boolean mkdirsInternal(Path path, FsPermission fsPermission) throws IOException {
        ...
        java.nio.file.Path nioDirPath = jimfs.getPath(path.toString().substring(path.toString().indexOf(":") + 1));
    ...
    public FileStatus getFileStatusInternal(Path path) throws IOException {
        ...
        if (Files.exists(nioFilePath)) {
            printAllFile ();
            return new FileStatus(Files.size(nioFilePath), Files.isDirectory(nioFilePath), 1, 1, 0, 0,
        ...
        if (stageDirIndex >=0 && pathStr.substring(stageDirIndex).split("/").length == 1) {
            ...
            printAllFile ();
            return new FileStatus(1, true, 1, 1, 0, 0,
                    new FsPermission((short) 777), null, null, path);
        }
        printAllFile ();
