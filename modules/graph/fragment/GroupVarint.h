...
#include "graph/fragment/Range.h"
 ...
 public:
  ...
  void finish() {
    if (count_) {
      ...
      // out_(std::string(tmp_, 0, Base::partialSize(buf_, count_)));
      count_ = 0;
    }
  }
...
class GroupVarintDecoder {
 public:
  ...
  const char* get_pos() const { return p_; }
  const size_t get_remain() const { return remaining_; }
